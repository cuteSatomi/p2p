package com.zzx.p2p.business.util;
...
public class CalculatetUtil {
     ...
     * @param yearRate         年利率
     * @param months2Return    还款期限
     * @return
     */
    public static BigDecimal calTotalInterest(int returnType, BigDecimal bidRequestAmount, BigDecimal yearRate, int months2Return) {
        ...
        if (returnType == BidConst.RETURN_TYPE_MONTH_INTEREST_PRINCIPAL) {
            ...
            if (months2Return == 1) {
                ...
                BigDecimal temp2 = (BigDecimal.ONE.add(monthlyRate)).pow(months2Return);
                BigDecimal temp3 = (BigDecimal.ONE.add(monthlyRate)).pow(months2Return).subtract(BigDecimal.ONE);
                // 算出每月还款
                BigDecimal monthToReturnMoney = temp1.multiply(temp2).divide(temp3, BidConst.CAL_SCALE, RoundingMode.HALF_UP);
                // 算出总还款
                BigDecimal totalReturnMoney = monthToReturnMoney.multiply(new BigDecimal(months2Return));
                // 算出总利息
            ...
            totalInterest = monthlyInterest.multiply(new BigDecimal(months2Return));
        }
     ...
     *
     * @param bidRequestAmount 借款金额
     * @param months2Return   还款期数
     * @param yearRate         年利率
     * @param returnType       还款类型
     * @param acturalBidAmount 投标金额
     * @return
     */
    public static BigDecimal calBidInterest(BigDecimal bidRequestAmount, int months2Return, BigDecimal yearRate, int returnType, BigDecimal acturalBidAmount) {
        // 借款产生的总利息
        BigDecimal totalInterest = calTotalInterest(returnType, bidRequestAmount, yearRate, months2Return);
        // 所占比例
